...
function backInUp(elem, ms, display = 'inline-block') {
	if (!elem)
		return;
	elem.children[0].classList.remove("animate__fadeOutDown");
	fadeIn(elem, 300, 'flex');
	setTimeout(() => {
		elem.children[0].classList.remove("d-none");
		elem.children[0].style.opacity=1;
		elem.children[0].classList.add("d-flex","animate__animated","animate__fadeInUp");
	}, 400);
	setTimeout(() => {
		elem.children[0].classList.remove("animate__fadeInUp");
	},1400);
}
function backInDown(elem, ms, display = 'inline-block') {
	...
	elem.children[0].classList.add("animate__fadeOutDown");
	setTimeout(() => {
		fadeOut(elem, ms);
	}, 500);
}
...
function shareURL(obj) {
	...
	switch (media) {
		case 'facebook':
			...
			break;
		case 'linkedin':
			window.open(`https://www.linkedin.com/shareArticle?mini=true&url=${url}&title=${title}&summary=${description}&source=emplicheck.com`, '_blank');
			break;
		case 'whatsapp':
			window.open(`https://${window.isMobile()?'api':'web'}.whatsapp.com/send?text=${title} ${description} ${url}`);
			break;
		default:
			if (window.isMobile()) {
				if (navigator.share)
